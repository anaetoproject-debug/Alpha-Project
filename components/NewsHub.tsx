...
import { getLatestCryptoNews } from '../services/cmcService';
...
const NewsHub: React.FC<NewsHubProps> = ({ theme }) => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      setIsLoading(true);
      try {
        // We bypass Gemini for News to save quota for Security Features
        const latestNews = await getLatestCryptoNews();
        setNewsList(latestNews || MOCK_NEWS);
      } catch (err) {
    ...
    fetchNews();
    const interval = setInterval(fetchNews, 600000); // 10-minute refresh
  ...
  const headline: NewsItem = newsList[0] || MOCK_NEWS[0];
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="flex-1">
            <h2 className={`text-3xl sm:text-5xl font-black italic uppercase tracking-tighter mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Global <span className="text-blue-500">Intelligence</span>
            </h2>
            <div className={`flex items-center gap-3 py-2 px-4 rounded-full border w-fit ${isDark ? 'bg-white/5 border-white/10' : 'bg-blue-50 border-blue-100'}`}>
              ...
              <p className={`text-[10px] sm:text-xs font-bold uppercase tracking-widest ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                Status: <span className={`font-medium normal-case ${isDark ? 'opacity-70' : 'text-slate-700'}`}>Bridge Frequencies Synchronized</span>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 mb-2">
            <span className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'opacity-30' : 'text-slate-500 opacity-60'}`}>Live Data Ingestion</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               ...
               {isLoading ? (
                 <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-20">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white">Ingesting Protocol News...</span>
                    </div>
                 </div>
               ) : (
            ...
            <div className="p-8 sm:p-10 relative">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2 bg-blue-500/10 px-3 py-1 rounded-full border border-blue-500/20">
                   ...
                   <span className="text-[8px] font-black text-blue-500 uppercase tracking-widest">Market Direct</span>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 rounded-[40px] border flex-1 ${getContainerStyles()}`}>
               ...
               <div className="space-y-8">
                  {newsList.slice(1, 4).map((item, i) => (
                    <div key={i} className="flex flex-col gap-2 group cursor-pointer" onClick={() => item.url && item.url !== '#' && window.open(item.url, '_blank')}>
            ...
            <div className={`p-8 rounded-[40px] border bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-xl group overflow-hidden relative`}>
               ...
               <p className="text-[10px] font-black uppercase tracking-[0.4em] mb-4 text-white/40 relative z-10">Bridge Operations</p>
               <h4 className="text-xl font-black italic uppercase tracking-tighter mb-4 leading-tight relative z-10">Data Streams Online.</h4>
               <p className="text-xs font-bold text-white/80 mb-8 leading-relaxed italic relative z-10">
                 Market frequencies are monitored via direct protocol ingestion for zero-latency intelligence.
               </p>
